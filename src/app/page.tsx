  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-gray-950 via-gray-900 to-gray-950 flex flex-col items-center justify-center text-white">
      <div className="text-center mb-8">
        ...
        <h1 className="text-5xl font-bold text-amber-400 mb-2 tracking-wider">구운록</h1>
        <p className="text-base text-gray-400 mt-3 tracking-wide">느리게, 그러나 끝까지</p>
      ...
      <div className="text-center text-gray-500 text-sm max-w-md mb-8 leading-relaxed">
        {deathCount > 0 ? (
          <>
            <p className="text-gray-400 italic">같은 산길이 또다시 펼쳐졌다.</p>
            <p className="text-gray-500 mt-2">{`익숙한 바람결\u2026 나는 다시 이곳에 서 있다. (${deathCount}번째 되풀이)`}</p>
          </>
        ) : (
          <>
            <p className="text-gray-400">나는 천재가 아니다.</p>
            <p className="text-gray-400">하지만 포기하지 않는 법은 배웠다.</p>
            <p className="mt-3 text-gray-500">남이 한 번에 깨우치는 수를, 나는 열 번 갈고</p>
            <p className="text-gray-500">남이 가볍게 넘기는 합을, 나는 백 번 되새긴다.</p>
          </>
        )}
      </div>

      <div className="w-full max-w-sm mb-8">
        <label className="block text-center text-amber-300 text-sm mb-3">무와 협을 좋아하는 당신은 누구인가?</label>
        <input
      ...
      <button onClick={handleStart}
        ...
        <span className="relative z-10">{deathCount > 0 ? "다시 발걸음을 옮기다" : "강호에 발을 딛다"}</span>
      ...
      <div className="absolute bottom-4 text-xs text-gray-700">{"구운록 v3.0 \u2014 Next.js Web"}</div>
    </div>
  );
}
